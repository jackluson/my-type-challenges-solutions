...
type LengthOfString<
  Type extends string,
  Counter extends ReadonlyArray<s> = []
> = Type extends `${s}${s}${s}${s}${s}${s}${s}${s}${s}${s}${infer Past10}`
  ? LengthOfString<Past10, [...Counter, ...ConstStringArr]>
  : Type extends `${infer _First}${infer Rest}`
  ? LengthOfString<Rest, ['', ...Counter]>
  : Counter['length'];
